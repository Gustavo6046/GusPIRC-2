  ...
  def read_output():
   while p.returncode is None:
    for line in iter(p.stdout.readline,b''):
     conn.send_message(chan,"OUT CID={} :{}".format(cid,line.decode("utf-8",errors="ignore")))
    for line in iter(p.stderr.readline,b''):
     conn.send_message(chan,"ERR CID={} :{}".format(cid,line.decode("utf-8",errors="ignore")))
...
def website_to_autodict(dictionary,url):
 for s in webget(url):
  for l in certcode(s,"ascii","[?]").splitlines():
   dictionary.parse(s)
def crawl_to(url,feed_func,_max_level,crawl_regex,ex_regex,_verbose=False):
 if not crawl_regex:
...
def define_commands(c):
 global markov,relateword,crawl,warzone_port,_num_bots,turn_index,autodict_enabled,condense_bot_messages,listeners_enabled,when_events,cleverbot,listeners,relay_msgs
 markov=reload(markov)
 relateword=reload(relateword)
 crawl=reload(crawl)
 cleverbot=reload(cleverbot)
 mch=markov.MarkovChain()
 clever=cleverbot.Cleverbot("GusBot3")
 try:
 ...
 relay_msgs=RegexTable({r"[\1 \3] * \2 \5":r"({}): :([^!]+)![^@]+@[^ ]+ PRIVMSG ({}) :(\x01)ACTION (.+)(\x01)",r"[\1 \3] <\2> \4":r"({}): :([^!]+)![^@]+@[^ ]+ PRIVMSG ({}) :((?!(\x01)ACTION).+)",r"[\1] \2 joined \3":r"({}): :([^ ]+) JOIN :({})",r"[\1] \2 left \3 (\4)":r"({}): :([^!]+)![^ ]+ PART ({}) :(.+)",r"[\1] \2 has quit (\3)":r"({}): :([^!]+)![^ ]+ QUIT :(.+)",r"[\1] \2 has kicked \4 from \3 (Reason: \3)":r"({}): :([^!]+)![^ ]+ KICK ({}) ([^ ]+) :(.+)",r"[\1] \2 sets ban on \4 on \3":r"({}): :([^ ]+) BAN ({}) (.+)",r"[\1] \2 changes nick to \3":r"({}): :([^!]+)![^ +] NICK :(.+)",})
 last_message={}
 ...
 @c.command("clever (.+)",0,help_string="Chat with CleverBot!")
 def cleverbot_talk(conn,msg,custom):
  return clever.ask(custom[0])
 @c.command(r"as ([^ ]+) (.+)",200,help_string="Transfer to someone... hue")
 def run_as(conn,msg,custom):
  user=custom[0]
  conn.emulate(":{} PRIVMSG {} :|;{}".format(user,msg.message_data[3],custom[1]))
 ...
 def server_emulation(conn,msg,custom):
  ...
  if server not in c.name_connections:
   ...
   if chan:
    c.connections[c.name_connections[server]].emulate(":{} PRIVMSG {} :|;{}".format("{}!{}@{}".format(*msg.message_data[:3]),chan,custom[1]))
   else:
    c.connections[c.name_connections[server]].emulate(":{} PRIVMSG GusBot3 :|;{}".format("{}!{}@{}".format(*msg.message_data[:3]),custom[1]))
 @c.command("who (.+)",0,help_string="Who is there? Query the who command!")
 ...
 @c.command("udict (.+)",0,help_string="Urban Dictionary now!")
 def urban_dict_addict(conn,msg,custom):
  results=[]
  ...
  for l in sorted(query["list"][:5],key=lambda x:x["thumbs_up"]-x["thumbs_down"]):
   results.append("Definition for '{}' (#{}) by {} (up: {}, down: {}): '{}' | Example: '{}'".format(l["word"].encode("utf-8"),l["defid"],l["author"].encode("utf-8"),l["thumbs_up"],l["thumbs_down"],l["definition"].replace("\n"," ").replace("\r"," ").encode("utf-8"),l["example"].replace("\n"," ").replace("\r"," ").encode("utf-8")))
  final=[]
  for r in results:
   sub=[]
   for w in r.split(" "):
 ...
 def py_eval(conn,msg,custom):
  try:
   __r=reprexc(custom[0],conn=conn,msg=msg,custom=custom)
   return repr(__r)
  except BaseException as err:
   traceback.print_exc()
 ...
 def run_shell(conn,msg,custom):
  ...
  t=threading.Thread(target=run_command,args=(conn,msg.message_data[3],custom[0]))
  t.daemon=True
  t.start()
  return m
 ...
 def get_markov(connection,message,custom_groups):
  ...
  return "{}: {}".format(message.message_data[0],r)
 fl=re.compile(" [ \t]+$")
 ...
 def echo(conn,msg,custom):
  return custom[0]
 @c.command("pipecrawl ([^ ]+) ([^ ]+) ([^ ]+) (\d+) (.+)",240,help_string="Pipe crawling into some command! Syntax: pipecrawl <command to pipe to> <URL crawling regex> <URL crawling exclusion regex> <max depth> <URL to crawl [<URL to crawl> [...]]>.")
 def pipe_crawling(conn,msg,custom):
  print "Crawling..."
  host="{}!{}@{}".format(*msg.message_data[0:3])
  def crawled(soup,url,level):
   conn.emulate(":{} PRIVMSG {} :{}{} {}".format(host,msg.message_data[3],c.command_prefix,custom[0],url))
  try:
   max_level=min(4,int(custom[3]))
  except ValueError:
   max_level=2
  for url in custom[4].split(" "):
   crawl_to(url,crawled,max_level,custom[1],custom[2])
 ...
 def chat_parse_file(conn,msg,custom):
  chatter.train([certcode(x,"ascii","[?]")for x in open(custom[0]).readlines()])
 ...
 def generic_message(connection,message,custom_groups):
  ...
  if custom_groups[1].startswith(c.command_prefix):
   return
  mch.read("_markov.cson")
  mch.receive(custom_groups[0])
  mch.write("_markov.cson")
  a=certcode(custom_groups[1],"ascii","[?]")
  if custom_groups[0]in last_message:
   try:
    chatter.train([last_message[custom_groups[0]],a])
   except ValueError:
    pass
  else:
 ...
 def list_commands(conn,msg,custom):
  ...
  return["Do |;help <command> to get command's help string as provided by it's module. Commands: {}".format(", ".join([f.cmd_name for f in r if hasattr(f,"help_string")and re.match(filt,f.cmd_name)]))]
 ...
 def command_help(conn,msg,custom):
  for cmd in conn.all_receivers():
   if hasattr(cmd,"cmd_name")and cmd.cmd_name.lower()==custom[0].lower():
    return "Help for '|;{}': '{}'; Regex syntax: '{}'; Minimum permission: {}.".format(custom[0],cmd.help_string,cmd.regex,cmd.min_perm)
 ...
 def statistical_leaderboard(conn,message,custom):
  ...
  try:
   leaderboard={"Lightcaster":(sorted(user_data.items(),key=lambda x:x[1]["stats"]["Infections"])[0],"enlightenings","Infections"),"Spender":(sorted(user_data.items(),key=lambda x:x[1]["stats"]["Buys"])[0],"buys","Buys"),"Assassin":(sorted(user_data.items(),key=lambda x:x[1]["stats"]["Kills"])[0],"kills","Kills"),"Survivor":(sorted(user_data.items(),key=lambda x:x[1]["stats"]["Turns Survived"])[0],"turns survived","Turns Survived"),"AIDS Killer":(sorted(user_data.items(),key=lambda x:x[1]["stats"]["AIDS Overdoses"])[0],"overdoses caused by them","AIDS Overdoses"),"Medic":(sorted(user_data.items(),key=lambda x:x[1]["stats"]["Cures"])[0],"\'unmushings\'","Cures"),}
  except IndexError:
 ...
 def smw_add_a_bot(conn,message,custom):
  ...
  for i in xrange(bots):
   ...
   try:
    ...
    return "No bot name to use! (use |;smw_addnames to add some bot names :D)"
   while name.lower()in user_data.keys():
    name+=str(user_data.keys().count(name.lower()))
   names.append(name)
   turns.append(name.lower())
   user_data[name.lower()]={"host":".!.@.","money":30,"mush":is_mush,"spores":6,"immune":3,"ammo":{x:y["defaultvalue"]for x,y in ammo_data.items()},"weapons":{x:False for x in weapon_data.keys()},"capsname":message.message_data[0],"health":100,"bot":True,"fungushealth":72,"stats":{"Kills":0,"AIDS Overdoses":0,"Infections":0,"Buys":0,"Turns Survived":0,"Cures":0,},}
 ...
 def join_warzone_stats(conn,message,custom):
  ...
  user_data[user.lower()]={"host":"{}!{}@{}".format(*message.message_data[:3]),"money":30,"mush":is_mush,"spores":6,"immune":3,"ammo":{x:y["defaultvalue"]for x,y in ammo_data.items()},"weapons":{x:False for x in weapon_data.keys()},"capsname":message.message_data[0],"health":100,"bot":False,"fungushealth":72,"stats":{"Kills":0,"AIDS Overdoses":0,"Infections":0,"Buys":0,"Turns Survived":0,"Cures":0,},}
 ...
 def smw_surrender(conn,message,custom):
  ...
  return["You surrender. You want your friends back. You are fed of doing this. You aren't meant to be evil. You are just scared.","Suddenly you feel like something's watching you. Like a heart has been touched, at last."]
 @c.command("smw_status( (.+))?",help_string="Your status at Sentient Mushes: Warzone.")
 def smw_user_status(conn,message,custom):
  user=message.message_data[0]
  if user.lower()not in user_data.keys():
   return["Join the game first using ||smw_join !"]
  return " | ".join(["{} Status:".format(user),"You have {} money.".format(user_data[user.lower()]["money"]),"You are{}a mush.".format((" not " if not user_data[user.lower()]["mush"]else " ")),"You have {} health{}.".format(user_data[user.lower()]["health"],(" ({} fungus health)".format(user_data[user.lower()]["fungushealth"])if user_data[user.lower()]["mush"]else "")),"You have immune system of {} in the Rehermann scale.".format(user_data[user.lower()]["immune"]),("You have the following weapons: "+", ".join(["{} ({})".format(weapon,user_data[user.lower()]["ammo"][weapon_data[weapon]["ammotype"]])for weapon,data in user_data[user.lower()]["weapons"].items()if data])if True in user_data[user.lower()]["weapons"].values()else "You have no guns! :("),("" if not user_data[user.lower()]["mush"]else "You have {} spores.".format(user_data[user.lower()]["spores"]))])
 ...
 def smw_get_user_status(conn,message,custom):
  ...
  return " | ".join(["{} Status:".format(target),"He/she has {} money.".format(user_data[target.lower()]["money"]),"He/she is{}a mush.".format((" not " if not user_data[target.lower()]["mush"]else " ")),"He/she has {} health{}.".format(user_data[target.lower()]["health"],(" ({} fungus health)".format(user_data[target.lower()]["fungushealth"])if user_data[target.lower()]["mush"]else "")),"He/she has immune system of {} in the Rehermann scale.".format(user_data[target.lower()]["immune"]),("He/she has the following weapons: "+", ".join(["{} ({})".format(weapon,user_data[target.lower()]["ammo"][weapon_data[weapon]["ammotype"]])for weapon,data in user_data[target.lower()]["weapons"].items()if data])if True in user_data[target.lower()]["weapons"].values()else "He/she has got no guns!"),]+([]if not user_data[target.lower()]["mush"]else["He/she has {} spores.".format(user_data[target.lower()]["spores"])]))
 ...
 def smw_about(conn,message,custom):
  return["Sentient Mushes: Warzone (c) 2016 Gustavo Ramos \"Gustavo6046\" Rehermann.    Source code is MIT License.","Source code at: https://www.github.com/Gustavo6046/GusBot-2 .   Sentient Mushes series; Inspired by another Twinoidian game."]
